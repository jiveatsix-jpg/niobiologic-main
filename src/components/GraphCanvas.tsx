...
export const GraphCanvas = React.memo(() => {
  ...
  const drawLegend = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    let currentY = 70; // Moved down to make room for the graph title
    const itemGap = 18;
    const iconSize = 7;
    const textOffset = 11;
    const lineHeight = 13;
    ...
    routes.forEach((route) => {
      ...
      ctx.font = `${scaledFont(uiSettings.fontSize - 4)} ${uiSettings.fontFamily}`;
  ...
  const drawEvolutionMode = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    ctx.font = `${scaledFont(uiSettings.fontSize)} ${uiSettings.fontFamily}`;
    ctx.fillStyle = uiSettings.baseColor;
    ...
    if (uiSettings.showSectionLabels) {
      sections.forEach((sec, i) => {
        const x = getX(i);
        ctx.save();
        ctx.translate(x, CANVAS_HEIGHT - PADDING + 24);
    ...
    if (tooltip && viewMode === 'EVOLUTION') {
      ...
      const boxHeight = (uiSettings.fontSize * ELEMENT_SCALE) + boxPaddingV * 2;
    ...
    ctx.fillStyle = uiSettings.baseColor;
    ctx.globalAlpha = 0.8;
    ctx.font = `bold ${scaledFont(uiSettings.fontSize - 6)} ${uiSettings.fontFamily}`;
    
    if (uiSettings.yAxisTitle) {
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText(uiSettings.yAxisTitle.toUpperCase(), PADDING - 10, PADDING - 18 + 6);
    }
    ...
    if (uiSettings.xAxisTitle) {
      ...
      lines.forEach((line, i) => {
        ctx.fillText(line, CANVAS_WIDTH - PADDING / 2 + 10, (CANVAS_HEIGHT - PADDING) + (i * 10));
      });
    }
    ...
    ctx.beginPath();
    ctx.moveTo(PADDING, PADDING - 18); // Tip
    ctx.lineTo(PADDING - 5, PADDING - 18 + 10); // Left wing
    ctx.lineTo(PADDING, PADDING - 18 + 7); // Back notch
    ctx.lineTo(PADDING + 5, PADDING - 18 + 10); // Right wing
    ctx.closePath();
    ctx.fill();
    ...
    ctx.lineTo(CANVAS_WIDTH - PADDING / 2 - 10, CANVAS_HEIGHT - PADDING - 5); // Top wing
    ctx.lineTo(CANVAS_WIDTH - PADDING / 2 - 7, CANVAS_HEIGHT - PADDING); // Back notch
    ctx.lineTo(CANVAS_WIDTH - PADDING / 2 - 10, CANVAS_HEIGHT - PADDING + 5); // Bottom wing
    ctx.closePath();
    ctx.fill();
  ...
  const drawComparisonMode = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    if (uiSettings.showCompYAxis) {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        const y = getY(val);
        ctx.fillText(val.toFixed(2), PADDING - 16, y);
        ctx.beginPath();
    ...
    sections.forEach((sec, i) => {
      ...
      if (uiSettings.showSectionLabels) {
        ctx.save();
        ctx.fillStyle = '#ffffff';
        ctx.font = `bold ${isSelected ? 8 : 7}px ${uiSettings.fontFamily}`;
        ctx.textAlign = 'center';
        ctx.shadowColor = isSelected ? '#ffffff' : sec.shadowColor;
        ctx.shadowBlur = isSelected ? 10 : 4;
        ctx.globalAlpha = 1.0;
        ctx.fillText(sec.name, xCenter, CANVAS_HEIGHT - PADDING + 19);
        ctx.restore();
      }
    });
  ...
  const drawDistributionMode = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    ctx.font = `bold 13px ${uiSettings.fontFamily}`;
    ctx.fillText(status, centerX, centerY - 8);
    
    ctx.fillStyle = '#00ffcc';
    ctx.font = `bold 9px ${uiSettings.fontFamily}`;
    ctx.fillText("SYNC: STABLE", centerX, centerY + 11);
    
    ctx.fillStyle = '#ffffff';
    ctx.font = `bold 8px ${uiSettings.fontFamily}`;
    ctx.shadowColor = sections[currentSectionIndex]?.color || '#00ffcc';
    ctx.shadowBlur = 5;
    ctx.fillText(`${sections[currentSectionIndex]?.name || ''} COMPOSITION`, centerX, centerY + 22);
    ctx.shadowBlur = 0;
  ...
  const drawRadarMode = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    for (let i = 0; i < numAxes; i++) {
      ...
      ctx.shadowBlur = 5;
      const textX = cx + Math.cos(angle) * (maxR + 18);
      const textY = cy + Math.sin(angle) * (maxR + 18);
